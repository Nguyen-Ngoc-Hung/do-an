...
@Service
public class DonViCPServiceImp implements DonViCPService {
    ...
    @Override
    public Map<String, Object> getDvcpPaging(RequestDto requestDto) {
        Map<String,Object> data = new HashMap<>();
        List<DonViCP> list = new ArrayList<DonViCP>();
        Long total = 0L;
        ...
        Specification<DonViCP> specification = new Specification<DonViCP>() {
            private static final long serialVersionUID = 1L;

            @Override
            public Predicate toPredicate(Root<DonViCP> root, CriteriaQuery<?> query,CriteriaBuilder criteriaBuilder) {
                List<Predicate> predicates = new ArrayList<>();

                return criteriaBuilder.and(predicates.toArray(new Predicate[predicates.size()]));
            }
        };
        Page<DonViCP> page = this.donViCPRepository.findAll(specification, pageable);
